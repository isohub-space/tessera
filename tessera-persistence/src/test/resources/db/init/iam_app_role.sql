...
-- Establishes the three-role model so neither Flyway nor
-- the runtime datasource connects as a superuser — a superuser bypasses RLS
...
--
-- The superuser is used only here (role creation + granting iam_migrator the CREATE
-- right on the schema it migrates into).
