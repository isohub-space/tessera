...
/**
 * The wire form of an {@link AuthorizationGrant} in the shared store, and the codec between the
 * two.
 *
 * <p>Values are stored as JSON strings. The Quarkus Redis marshaller special-cases
 * {@link String} and writes its raw UTF-8 bytes, so the value on the server is exactly this JSON
 * — no schema to register, no codec to configure, and an operator inspecting a key sees something
 * readable.
 *
 * <p>The domain record is deliberately not the wire type. Keeping serialization concerns out of
 * {@code tessera-domain} is what the architecture test enforces, so the adapter owns this flat
 * projection and the mapping both ways.
 *
 * <p>{@link Instant}s are carried as (epoch-second, nano) pairs rather than millis so the
 * round-trip is lossless: a truncated {@code expiresAt} would move a code's expiry, and moving it
 * later is a (small) extension of the replay window.
 */
record StoredGrant(
        String tenantId,
        String baselineId,
    ...
    /**
     * Decodes a stored value back into a grant.
     *
     * @param json the stored JSON value (never {@code null})
     * @return the grant, or {@code null} if the value is not a decodable grant — a malformed or
     *         unknown-shaped entry is a miss, never an exception on the token endpoint. The
     *         atomic {@code GETDEL} has already taken the entry, so it cannot be retried either.
     */
    static AuthorizationGrant decode(String json) {
        ...
        try {
            return new AuthorizationGrant(
                    ...
                    stored.subjectId,
                    stored.redirectUri,
                    // LinkedHashSet, not Set.copyOf: the grant's scope order is preserved on the
                    // round-trip exactly as the issuing service recorded it.
            ...
            // A structurally valid JSON object that is not a valid grant (bad UUID, unknown PKCE
            // method, missing field, expiresAt not after issuedAt). Same answer: a miss.
            return null;
        }
    }
}
